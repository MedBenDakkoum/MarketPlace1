      ...
      aggregate([
        {
          $group: {
            ...
            hasPending: {
              $max: {
                $cond: { if: { $eq: ["$status", "PENDING"] }, then: true, else: false }
              }
            },
            numT:{$sum:1}
          }
...
async function getTransactions(id) {
    try{
      return new Promise(async (resolve,reject)=>{
        await Transaction.find({userId:id})
        .then((transactions)=>{
    ...
    try{
      return new Promise(async (resolve,reject)=>{
        ...
        if(!user){
          ...
          if(user.balance>=100){
            let pendingTransactions = await Transaction.find({userId:id,status:"PENDING"});
            if(!pendingTransactions.length>0){
              let settings = await settingsService.getSettings();
              let adminAmount = 0;
              if(settings.Commision.type=="percentage"){
                adminAmount = (user.balance*parseFloat(settings.Commision.details))/100;
              }else{
                adminAmount = user.balance-parseFloat(settings.Commision.details)
              }
  ...
  async function confirmWithdrawl(id) {
    try{
      return new Promise(async (resolve,reject)=>{
        await Transaction.find({userId:id,status:"PENDING"})
        .then(async (transactions)=>{
          let pendingTransaction = transactions[0];
          let user = await User.findById(pendingTransaction.userId);
          await user.update({balance:(user.balance-pendingTransaction.fullAmount).toFixed(2)})
          .then(async (rslt)=>{
            await Transaction.findOneAndUpdate({userId:id,status:"PENDING"},{status:"ACCEPTED"})
            .then((r)=>{
              resolve("done")
            })
            .catch((e)=>{

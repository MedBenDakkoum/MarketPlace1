...
import { CFormTextarea,CFormSelect,CFormInput,CButton,CTable,CTableRow,CTableHeaderCell,CTableDataCell,CTableHead ,CTableBody} from '@coreui/react';
...
const order ={
    ...
    messages:[{
        ...
        messageContent:"Unfortunately, an item on your order is currently out of stock. This may cause a slight delay in delivery.Please accept our apologies and rest assured that we are working hard to rectify this.",
...
function SingleOrder() {
    ...
    useEffect(()=> {
        let rows1 = order.products.map((element, index) => (
            <CTableRow key={element.id} color='light'>
                ...
                <CTableDataCell><a onClick={()=>{navigate('/admin/products/'+element.ref)}} style={{cursor:"pointer",color:"blue"}}>View Product</a></CTableDataCell>
            </CTableRow>
        ));
        setRows(rows1);
    },[])
    const handleChangeStatusDocs = (e)=>{
        ...
        setClasses({status:"",docs:"",[e.target.getAttribute('name')]:"active-bar"})
    }
                ...
                <div className="customer-card">
                    ...
                    <div className="customer-name">
                        <div className="customer-name-card">
                            <BsPersonFill/>{order.customer.name}
                        </div>
                        ...
                        <div className="customer-address">
                            <p name="customer-address"><span>Line 1:</span>{order.customer.address.line1}</p>
                            <p name="customer-address"><span>Line 2:</span>{order.customer.address.line2}</p>
                            <p name="customer-address"><span>City:</span>{order.customer.address.city}</p>
                            <p name="customer-address"><span>State:</span>{order.customer.address.state}</p>
                            <p name="customer-address"><span>Country:</span>{order.customer.address.country}</p>
                            <p name="customer-address"><span>ZipCode:</span>{order.customer.address.zipCode}</p>
                        </div>
                    </div>
                </div>
            ...
            <div className="order-right">
                        ...
                        <CTableBody>
                            ...
                            <CTableRow key="add" color='light'>
                                ...
                                <CTableDataCell colSpan={2}>
                                    <CFormInput name="ref" type="text" id="inputRef" label="Reference"/>
                                </CTableDataCell>
                                <CTableDataCell>
                                    <CFormInput name="quantity" type="text" id="inputQuantity" label="Quantity"/>
                                </CTableDataCell>
                                <CTableDataCell></CTableDataCell>
                                <CTableDataCell></CTableDataCell>
                                <CTableDataCell><CButton type="button">Add</CButton></CTableDataCell>
                            </CTableRow>
                        </CTableBody>
                    </CTable>
                    <CTable>
                ...
                <div className="info-container">
                    <div className="info-navbar">
                        <p onClick={handleChangeStatusDocs} name="status" className={classes.status}>
                            Status
                        </p>
                        <p onClick={handleChangeStatusDocs} name="docs" className={classes.docs}>
                            Documents
                        </p>
                    </div>
                    <div className="info-content">
                        ...
                        <CTable>
                            <CTableBody>
                                <CTableRow key="add" color='light'>
                                    ...
                                    <CTableDataCell><a style={{cursor:"pointer",color:"blue"}}>Resend Email</a></CTableDataCell>
                                </CTableRow>
                            </CTableBody>
                        </CTable>
                        <div className="update-status">
                            <CFormSelect name="status" defaultValue="delievered">
                                    <option value="sellconfirm">Awaiting Seller Confirmation</option>
                                    <option value="suppconfirm">Awaiting Supplier Confirmation</option>
                        ...
                        <>
                                ...
                                <CTableBody>
                                    ...
                                    <CTableRow color='light'>
                                        <CTableDataCell colSpan={3}>
                                            <CFormInput name="quantity" value={invoice.note} type="text" id="inputQuantity" />
                                        </CTableDataCell>
                                        <CTableDataCell>
                            ...
                            : <CButton onClick={handleGenerateInvoice} type="button">Generate Invoice</CButton> }
                        </>
                        : <h1>Not found</h1>
                        }
                    </div>
